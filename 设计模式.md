...
创建型模式主要关注如何创建对象，避免在创建对象时出现冗余和复杂的代码。它们将对象的创建过程与使用过程分离，从而使得代码更具可扩展性。
...
结构型模式关注如何组织和组合类和对象，使得它们的结构更加灵活，能够高效地处理系统的变化。
...
- **外观模式（Facade Pattern）**：为子系统中的一组接口提供一个统一的高层接口，使得子系统更容易使用。适用于需要简化复杂系统接口的场景。
